...
#        networks = neutron.list_networks(**_params)
    ...
    for network in networks["networks"]:
        ...
        table.title = ("Network ID: %s | "
                       "Project ID: %s | "
                       "Network Name: %s" % (BOLD + network["id"] + RST,
                                             BOLD + network["project_id"] + RST,
                                             BOLD + network["name"] + RST)
                      )
...
if __name__ == "__main__":
    ...
    # (todo) Allow user to specify network ID, project ID, or list all networks!

    parser = argparse.ArgumentParser(
        description="network.py - Utility to show IP address utilization"
    )
    group = parser.add_mutually_exclusive_group(required=False)
    group.add_argument(
        "--network-id",
        type=str,
        help="Provides IP address utilization for specified network",
        required=False,
        default=None,
    )
    group.add_argument(
        ...
        dest="allnetworks",
        action="store_true",
        help="Provides IP address utilization for all networks (default is external only)",
        required=False,
    )
